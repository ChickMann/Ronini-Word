...
public class InputDisplayManager : MonoBehaviour
{
    ...
    private void HandleCorrectInput(string inputVal)
    {
        ...
        if (isLastChar)
        {
            if (_isWrongInVocab)
            {
                GameDataManager.Instance.OnPlayerAnswered(_currentVocabData.VocabID,false);
                PixelTextController.Instance.GoodTextAnimation();
            }
            else
            {
                GameDataManager.Instance.OnPlayerAnswered(_currentVocabData.VocabID,true);
                PixelTextController.Instance.PrefectTextAnimation();
            }
    ...
    private List<string> GenerateCharPool()
    {
        ...
        var sourceData = JapaneseData.basicHiraAndKata.ToList(); 
